...
#pragma omp parallel num_threads(threadsNumber)	// Распараллеливание на четыре потока
	{
		...
		Decoding(std::ref(threadsData[i]));	// Вызываем в i-том потоке метод дешифровки
#pragma omp critical	// Критическая секция, блок выполняется по очереди всеми потоками
		{
			outputValues += threadsData[i].returnValue;	// Суммируем returnValue каждого потока
		}
	}
	...
	else
	{
		...
		std::cout << "Decoded text: " + decodedText;	// Вывод расшифрованной строки
	}
}
